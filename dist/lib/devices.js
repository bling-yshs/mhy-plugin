 ...
 */
export async function registerDeviceSession(cookie, device) {
    const key = createHash('sha256')
        .update(`${cookie}:${device.device_id}:${device.device_fp}`)
        .digest('hex');
    if ((registered.get(key) || 0) > Date.now())
        return;
 ...
 */
export async function getBoundDevice(accountId) {
 ...
 * @returns 已绑定设备
 */
export async function bindDevice(userId, accountId, info) {
    if (!(await ownsAccount(userId, accountId)))
        throw new Error('账号绑定已变更');
    ...
    if (!device)
        throw new Error('设备信息格式错误');
    if (device.android)
        device = await refreshDevice(device);
    const saved = device;
    await writeTransaction(async (transaction) => {
        if (revisions.get(accountId) !== revision)
            throw new Error('设备绑定已变更，请重试');
        if (!(await ownsAccount(userId, accountId)))
            throw new Error('账号绑定已变更');
        const [count] = await MysUserDB.update({ bound_device: JSON.stringify(saved) }, { where: { ltuid: accountId, bound_device: previous }, transaction });
        if (!count)
            throw new Error('设备绑定已变更，请重试');
    });
    return saved;
 ...
 * @returns 解除完成
 */
export async function unbindDevice(userId, accountId) {
    if (!(await ownsAccount(userId, accountId)))
        throw new Error('账号绑定已变更');
 ...
 * @returns 当前有效绑定设备
 */
export async function ensureDevice(accountId) {
    ...
    const job = (async () => {
        ...
        if (!device.android || (device.device_fp && (device.expiresAt || 0) > Date.now()))
            return device;
        const updated = await refreshDevice(device);
        const [count] = await writeTransaction((transaction) => MysUserDB.update({ bound_device: JSON.stringify(updated) }, { where: { ltuid: accountId, bound_device: raw }, transaction }));
        if (!count)
            throw new Error('设备绑定已变更，请重新查询');
        return updated;
    })();
    ...
    try {
        ...
        if (refreshing.get(accountId) === job)
...
export function parseDevice(info) {
    ...
    if (info.device_id !== undefined || info.device_fp !== undefined) {
        for (const key of ['device_id', 'device_fp']) {
            if (typeof info[key] !== 'string' || !/^[\x21-\x7e]{1,256}$/.test(info[key]))
                return false;
        }
 ...
 */
export async function refreshDevice(device, overseas = false) {
    const info = device.android;
    ...
    const response = await fetch(`https://${overseas ? 'sg-public-data-api.hoyoverse.com' : 'public-data-api.mihoyo.com'}/device-fp/api/getFp`, {
        method: 'POST',
        signal: AbortSignal.timeout(10000),
