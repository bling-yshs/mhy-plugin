 ...
 * @returns 更新的米游社账号 ID
 */
export async function setAutoSign(userId, game, enabled) {
    const accounts = (await getAccounts(userId)).filter(account => account.uids[game]?.length);
    if (!accounts.length)
        throw new Error(`请先绑定${gameNames[game]}的米游社账号`);
...
export async function signRole(ltuid, game, uid, manual) {
    ...
    try {
        ...
        if (!record)
            return `${label}：今天已执行自动签到`;
        const result = {};
        let message = '';
        try {
            const account = await MysUserDB.findByPk(ltuid);
            if (!account?.ck || !account.uids[game]?.includes(uid))
                throw new Error('账号或角色绑定已变更，请重新绑定');
            const context = { game, uid, accountId: ltuid, server: region };
            const info = await request('bbs_sign_info', context, {});
            result.info = info;
            if (info.retcode !== 0)
                throw new Error(info.message || `签到状态查询失败：${info.retcode}`);
            ...
            if (info.data.is_sign) {
                ...
                if (sign.retcode === -5003) {
                    ...
                    if (sign.retcode !== 0)
                        throw new Error(sign.message || `签到失败：${sign.retcode}`);
 ...
 * @returns 每个角色的签到结果
 */
export async function signUser(userId, game) {
    const messages = [];
    const seen = new Set();
 ...
 * @returns 是否已关闭
 */
async function stopFailedAutoSign(setting) {
    return writeTransaction(async (transaction) => {
        ...
        for (let index = 0; index < dates.length; index++) {
            if (Date.parse(dates[0]) - Date.parse(dates[index]) !== index * 86400000)
                return false;
            if (records.some(record => record.sign_date === dates[index] && record.status !== 'failed'))
                return false;
        }
...
export async function runAutoSign() {
    ...
    for (const setting of await AutoSignSettingDB.findAll({ where: { enabled: true } })) {
        if (!linked.has(String(setting.ltuid)))
            continue;
        if (!['gs', 'sr', 'zzz'].includes(setting.game))
            continue;
        if (await stopFailedAutoSign(setting))
            continue;
 ...
 * @returns 状态文本
 */
export async function getSignStatus(userId) {
    const lines = [];
    for (const account of await getAccounts(userId)) {
