    ...
    async probe() {
        ...
        try {
            ...
            if (!['gs', 'sr', 'zzz'].includes(game))
                throw new Error('game 只能是 gs、sr 或 zzz');
            ...
            if (!uid)
                throw new Error(`该账号没有 ${game} 游戏 UID，请在 JSON 中指定 uid`);
            const bodyParams = input.body && typeof input.body === 'object' && !Array.isArray(input.body) ? input.body : {};
            const params = {
                ...input.query,
                ...bodyParams,
                ...input.params,
            ...
            const response = await execute(input.operation, { game, uid, accountId: String(account.ltuid), cookie: account.ck }, params, { method: input.method, body: input.body, history: true });
            await this.reply(`${response.retcode === 0 ? '请求成功' : '接口已响应'}：${input.operation}，retcode=${response.retcode}。请求与响应已记录在 plugins/mhy-plugin/fetch-history。`);
        }
        catch (error) {
