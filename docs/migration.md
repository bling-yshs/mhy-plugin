...
账号和凭据保存在云崽配置指定的原 SQLite 数据库。`MysUsers`、`Users`、`UserGames` 模型由 mhy-plugin 管理，原神原导入路径继续有效。首次初始化时为 `MysUsers` 补充 `stoken`、`mid`、`login_device`、`bound_device`；SQLite 升级前通过 `VACUUM INTO` 保存一致性备份，文件名为原库路径加 `.before-mhy-时间戳.sqlite`。
...
4. 停止机器人后，根据需要预览并导入历史数据。
5. 启动机器人。正常启动沿用云崽 `cfg.db`；`MHY_DATABASE_PATH` 仅供临时库测试或离线迁移使用。
...
```powershell
node scripts/import-legacy.mjs --database '实际SQLite绝对路径'
node scripts/import-legacy.mjs --database '实际SQLite绝对路径' --apply
```
...
```powershell
node scripts/import-legacy.mjs --database '实际SQLite绝对路径' --device-source '米游社账号ID=miao'
node scripts/import-legacy.mjs --database '实际SQLite绝对路径' --device-source '米游社账号ID=zzz' --apply
```
...
- 各插件原始文件保存在本地 `integration-review/original`，与本次数据库备份分开。
