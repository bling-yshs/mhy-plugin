...
type Monitor = {
  busy: boolean
  send: (message: string) => Promise<object | boolean | undefined>
}
...
export class MhyServerMonitor extends plugin {
  ...
  async toggle(): Promise<boolean> {
    ...
    if (this.e.msg.includes('关闭')) {
      ...
      await this.reply(
        '开服监控已开启，每 30 秒检查一次，通知成功后自动停止。机器人重启后需重新开启。',
      )
    }
    return true
  }
  ...
  async poll(): Promise<void> {
    await Promise.all(
      [...monitors].map(async ([key, monitor]) => {
        if (monitor.busy) return
        monitor.busy = true
        try {
          const opened = await isGenshinServerOpen()
          if (!opened || monitors.get(key) !== monitor) return
          const sent = await monitor.send('原神新版本已开服')
          if (!sent) throw new Error('群消息发送失败')
          if (monitors.get(key) === monitor) monitors.delete(key)
        } catch (error) {
          logger.warn(
            `[mhy-plugin] 开服监控 ${key} 本轮失败，将在下轮重试：${error instanceof Error ? error.message : '未知错误'}`,
          )
        } finally {
          monitor.busy = false
        }
