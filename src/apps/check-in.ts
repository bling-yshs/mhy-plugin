...
export class MhyCheckIn extends plugin {
  ...
  async daily(): Promise<void> {
    if (signClock().time === '00:02') await runAutoSign()
  }
  ...
  async toggle(): Promise<boolean> {
    ...
    try {
      const ids = await setAutoSign(
        String(this.e.mainUserId || this.e.originalUserId || this.e.user_id),
        game,
        enabled,
      )
      await this.reply(
        `自动签到已${enabled ? '开启' : '关闭'}，米游社账号：${ids.join('、')}。\n每天北京时间 00:02 执行，失败后需手动签到。`,
      )
    } catch (error) {
  ...
  async sign(): Promise<boolean> {
    ...
    try {
      await this.reply(
        (
          await signUser(String(this.e.mainUserId || this.e.originalUserId || this.e.user_id), game)
        ).join('\n'),
      )
    } catch (error) {
  ...
  async status(): Promise<boolean> {
    await this.reply(
      await getSignStatus(String(this.e.mainUserId || this.e.originalUserId || this.e.user_id)),
    )
    return true
  }
}
