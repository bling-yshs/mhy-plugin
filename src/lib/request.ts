...
const fingerprints = new Map<string, { device: Device; expires: number }>()

/** 判断旧操作是否属于当前迁移范围。
 * @param operation 原有操作名
 * @returns 是否接管
 */
export function handlesOperation(operation: string): boolean {
 ...
 * @returns 区服
 */
export function getServer(uid: string, game: Game): string {
  if (game === 'zzz')
    return uid.length < 10
      ? 'prod_gf_cn'
      : { '10': 'prod_gf_us', '15': 'prod_gf_eu', '13': 'prod_gf_jp', '17': 'prod_gf_sg' }[
          uid.slice(0, -8)
        ] || 'prod_gf_cn'
  const index =
    ({ '5': 1, '6': 2, '7': 3, '8': 4, '18': 4, '9': 5 } as Record<string, number>)[
      uid.slice(0, -8)
    ] || 0
  return (
    game === 'gs'
 ...
 * @returns 完整米哈游响应
 */
export async function request<K extends keyof Operations>(
  operation: K,
  context: RequestContext,
  params: Operations[K]['params'],
): Promise<ApiResponse<Operations[K]['data']>> {
  return execute(operation, context, params) as Promise<ApiResponse<Operations[K]['data']>>
}
...
export async function execute(
  ...
  if (!cookie) throw new Error('请先绑定米游社账号')
  ...
  if (!device) {
    ...
    if (cached && cached.expires > Date.now()) device = cached.device
    else {
  ...
  if (op === 'getFp') {
    if (context.profile === 'zzz' && cn) await registerDeviceSession(cookie, device)
  ...
  if (
    (zzzProfile && op !== 'dailyNote' && op !== 'zzzExplorationDetail') ||
    (!cn && game === 'zzz' && op !== 'dailyNote')
  )
    query.set('lang', 'zh-cn')
  if (op === 'avatarInfo') query.set('need_wiki', String(params.need_wiki ?? true))
  ...
  if (params.query) {
    ...
    for (const [key, value] of entries) {
      ...
      } else query.set(key, value === null ? '' : String(value))
    }
  }
  let body = ''
  ...
  if (device.device_fp) headers.set('x-rpc-device_fp', device.device_fp)
  if (!headers.has('User-Agent'))
    headers.set(
      'User-Agent',
      `Mozilla/5.0 (Linux; Android ${device.android?.androidVersion || '12'}; ${device.android?.deviceModel || device.device_id}) AppleWebKit/537.36 Mobile Safari/537.36 ${cn ? 'miHoYoBBS' : 'miHoYoBBSOversea'}/${version}`,
    )
  headers.set(
    'Referer',
    cn
  ...
  if (zzzProfile) {
    ...
    if (device.android) {
      headers.set(
        'x-rpc-device_name',
        `${device.android.deviceFingerprint?.split('/')[0]} ${device.android.deviceModel}`,
      )
 ...
 * @returns 原响应或 false
 */
export async function legacyGetData(
  api: any,
  type: string,
  ...
  const hash = createHash('sha256')
    .update(
      JSON.stringify([game, api.uid, type, data, account?.ck || api.cookie, account?.bound_device]),
    )
    .digest('hex')
  ...
  try {
    ...
    ;(globalThis.logger || console).error(
      `[mhy-plugin] ${type} 请求失败：${error instanceof Error ? error.message : '未知错误'}`,
    )
    return false
  }
}
...
export async function getGameRoles(
  ...
  return response.ok ? ((await response.json()) as ApiResponse<any>) : false
}
