 ...
 * @returns 账号 ID 或空串
 */
export function accountIdFromCookie(cookie = ''): string {
  return (
    /(?:^|;\s*)(?:ltuid_v2|ltuid|account_id_v2|account_id|stuid)=([^;]+)/.exec(cookie)?.[1] || ''
  )
}
 ...
 * @returns 按绑定顺序排列的账号
 */
export async function getAccounts(userId: string): Promise<MysUserDB[]> {
  const user = await UserDB.findByPk(String(userId))
  const result: MysUserDB[] = []
 ...
 */
export async function resolveAccount(userId: string, game: Game): Promise<MysUserDB | undefined> {
  const user = await UserDB.findByPk(String(userId))
  const accounts = await getAccounts(userId)
  return (
    accounts.find((account) => account.uids?.[game]?.includes(String(user?.games?.[game]?.uid))) ||
 ...
 * @returns 用户 ID 列表
 */
export async function accountUsers(accountId: string): Promise<string[]> {
...
export async function saveLogin(
  ...
  const account = await writeTransaction(async (transaction) => {
    ...
    const account =
      (await MysUserDB.findByPk(login.accountId, { transaction })) ||
      MysUserDB.build({ ltuid: login.accountId })
    const uids: Record<Game, string[]> = { gs: [], sr: [], zzz: [] }
    for (const role of login.roles) {
      const game = ({ hk4e_cn: 'gs', hkrpg_cn: 'sr', nap_cn: 'zzz' } as const)[
        role.game_biz as 'hk4e_cn'
      ]
    ...
    const user =
      (await UserDB.findByPk(String(userId), { transaction })) ||
      UserDB.build({ id: String(userId), type: 'qq' })
    const ids = new Set<string>((user.ltuids || '').split(',').filter(Boolean))
    ids.add(login.accountId)
    const games = user.games
    for (const game of ['gs', 'sr', 'zzz'] as const)
      if (!games[game].uid && uids[game].length) games[game].uid = uids[game][0]
 ...
 * @returns 是否删除账号
 */
export async function unlinkAccount(userId: string, accountId: string): Promise<boolean> {
  const deleted = await writeTransaction(async (transaction) => {
    ...
    user.ltuids = (user.ltuids || '')
      .split(',')
      .filter((id: string) => id !== String(accountId))
      .join(',')
    await user.save({ transaction })
